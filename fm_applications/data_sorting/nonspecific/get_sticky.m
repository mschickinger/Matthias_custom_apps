function [ intervals, indices ] = get_sticky( RMS, XY, straj, segments, segmInds, threshs, cutoffD, sigmas, multSEM )
        ...
        for i = 1:size(states{k},1)
            ...
            if any(tmpB==1)
                if states{k}(i,2)>Lmin(k)
                 ...
                 %elseif sum(tmpB)/length(tmpB) > cutoffD(k)
