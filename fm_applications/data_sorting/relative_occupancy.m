...
for m = 1:size(hop.results,1)
    for s = 1:size(hop.results{m},1)
        ...
        for i = 1:size(hop.results{m}{s}.ex_int,1)
            tmp_ex = [tmp_ex hop.results{m}{s}.ex_int(i,1):hop.results{m}{s}.ex_int(i,2)]; % pre-allocation to complicated?
        end
        tmp_ex(tmp_ex>length(hop.results{m}{s}.state_trace)) = [];
        tmp_st = hop.results{m}{s}.state_trace;
        tmp_st(tmp_ex) = [];
...
for m = 1:size(hop.results,1)
    for s = 1:size(hop.results{m},1)
        counter = counter + 1;
        Nu(counter) = length(hop.results{m}{s}.Tu);
        Nb(counter) = length(hop.results{m}{s}.Tb);
    end
end
...
for p = 1:P
    ...
    for j = 1:num_on_page(p)
        ...
        hold on
        plot(tmult*(1:length(hop.results{spotident(ROb_I(i),1)}{spotident(ROb_I(i),2)}.state_trace)), ...
            hop.results{spotident(ROb_I(i),1)}{spotident(ROb_I(i),2)}.state_trace + st_y_offset, 'LineWidth', .1)
        if hop.results{spotident(ROb_I(i),1)}{spotident(ROb_I(i),2)}.arxv.max_frame < length(rms_cell{spotident(ROb_I(i),1)}{spotident(ROb_I(i),2)}.rms10)
            ...
            plot(tmult*(1:length(rms_cell{spotident(ROb_I(i),1)}{spotident(ROb_I(i),2)}.rms10)), ...
